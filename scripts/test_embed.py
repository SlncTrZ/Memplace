...
# Lay 1 point
r = requests.post(
    f"{QDRANT_URL}/collections/meilin_omniscience_wiki/points/scroll",
    headers=H,
...
r2 = requests.post(
    ...
    json={"model": "nomic-embed-text:latest", "prompt": c[:800].replace(chr(10), " ").strip()},
    timeout=45,
)
...
# PUT
r3 = requests.put(
    f"{QDRANT_URL}/collections/meilin_omniscience_wiki/points",
    headers=H,
    json={"points": [{"id": pid, "vector": v, "payload": p["payload"]}]},
    timeout=60,
)
