...
def embed(text):
    ...
    r = requests.post(f"{OLLAMA_URL}/api/embeddings", json={"model": EMBED_MODEL, "prompt": clean}, timeout=45)
    r.raise_for_status()
...
def process(col):
    ...
    while True:
        ...
        r = requests.post(f"{QDRANT_URL}/collections/{col}/points/scroll", headers=h(), json=payload, timeout=30)
        data = r.json()["result"]
        ...
        for p in points:
            ...
            if len(batch) >= 50:
                requests.put(f"{QDRANT_URL}/collections/{col}/points", headers=h(), json={"points": batch}, timeout=60)
